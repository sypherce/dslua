...
#define SET_BACKLIGHT_BOTTOM_ON  (0x1211B221)
#define SET_BACKLIGHT_TOP_ON     (0x1211B222)
#define SET_BACKLIGHT_BOTTOM_OFF (0x1211B223)
#define SET_BACKLIGHT_TOP_OFF    (0x1211B224)
...
inline void NDSX_SetBottomBacklight_On()
{ 
    while(!(REG_IPC_FIFO_CR & IPC_FIFO_SEND_EMPTY));
    REG_IPC_FIFO_TX = SET_BACKLIGHT_BOTTOM_ON;
    swiWaitForVBlank();
}

inline void NDSX_SetTopBacklight_On()
{ 
    while(!(REG_IPC_FIFO_CR & IPC_FIFO_SEND_EMPTY));
    REG_IPC_FIFO_TX = SET_BACKLIGHT_TOP_ON;
    swiWaitForVBlank();
}
...
inline void NDSX_SetBottomBacklight_Off()
{ 
    while(!(REG_IPC_FIFO_CR & IPC_FIFO_SEND_EMPTY));
    REG_IPC_FIFO_TX = SET_BACKLIGHT_BOTTOM_OFF;
    swiWaitForVBlank();
}

inline void NDSX_SetTopBacklight_Off()
{ 
    while(!(REG_IPC_FIFO_CR & IPC_FIFO_SEND_EMPTY));
    REG_IPC_FIFO_TX = SET_BACKLIGHT_TOP_OFF;
    swiWaitForVBlank();
}
...
inline bool NDSX_ARM7_BrightnessFifo(u32 msg)
{
    ...
    if(NDSX_ARM7_IsLite())
    {
        ...
        else if(msg == SET_BACKLIGHT_BOTTOM_ON)
        {
            u32 reg_with_backlight = readPowerManagement(PM_CONTROL_REG) | PM_BACKLIGHT_BOTTOM;
            writePowerManagement(PM_CONTROL_REG, reg_with_backlight);
            return true;
        }
        else if(msg == SET_BACKLIGHT_TOP_ON)
        {
            u32 reg_with_backlight = readPowerManagement(PM_CONTROL_REG) | PM_BACKLIGHT_TOP;
            writePowerManagement(PM_CONTROL_REG, reg_with_backlight);
            return true;
        }
        ...
        else if(msg == SET_BACKLIGHT_BOTTOM_OFF)
        {
            u32 reg_without_backlight = readPowerManagement(PM_CONTROL_REG) & ~PM_BACKLIGHT_BOTTOM;
            writePowerManagement(PM_CONTROL_REG, reg_without_backlight & 255);
            return true;
        }
        else if(msg == SET_BACKLIGHT_TOP_OFF)
        {
            u32 reg_without_backlight = readPowerManagement(PM_CONTROL_REG) & ~PM_BACKLIGHT_TOP;
            writePowerManagement(PM_CONTROL_REG, reg_without_backlight & 255);
            return true;
        }
