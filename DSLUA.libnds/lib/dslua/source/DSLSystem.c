...
{
	lua_pushnumber( lState, vblankCount);//Test this
	return 1;
}
...
{
	int i = 0;
   u8 Name[21]; u8 NameLength = INFO_NAME_LENGTH; for (i = 0; i < NameLength; i++) Name[i] = *(u8*)(0x027FFC86 + (i << 1)); Name[i] = 0;
...
{
	int i = 0;
   u8 Name[21]; u8 NameLength = INFO_MESSAGE_LENGTH; for (i = 0; i < NameLength; i++) Name[i] = *(u8*)(0x027FFC9C + (i << 1)); Name[i] = 0;
   lua_pushstring( lState, Name );
   return 1;
}
